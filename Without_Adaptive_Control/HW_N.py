...
def plot_results(times: np.ndarray, positions: np.ndarray, velocities: np.ndarray, torque: np.ndarray, error: np.ndarray):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/HW_ADP_N_pos.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/HW_ADP_N_vel.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/HW_ADP_N_tor.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/HW_ADP_N_err.png')
    plt.close()
...
def main():
    ...
    sim = Simulator(
        ...
        show_viewer=True,
        record_video=True,
        video_path="logs/videos/HW_ADP_N.mp4",
        fps=30,
        width=1920,
        height=1080
    )
